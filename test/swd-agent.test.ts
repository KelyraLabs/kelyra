...
describe('external-agent SWD API', () => {
  ...
  it('applies external-agent FILE_ACTION text without any model API key', async () => {
    await withTempProject('kelyra-byoa-apply-', async () => {
...
`;
  ...
  it('dry-runs external-agent actions without touching disk or saving receipts', async () => {
    await withTempProject('kelyra-byoa-dryrun-', async () => {
      const result = await applyExternalAgentActions({
  ...
  it('fails closed on sensitive files and never writes them', async () => {
    await withTempProject('kelyra-byoa-sensitive-', async () => {
      const result = await applyExternalAgentActions({
        rawInput: JSON.stringify({
          actions: [{
            path: '.env',
  ...
  it('requires explicit allowRisky for high-impact command-surface files', async () => {
    await withTempProject('kelyra-byoa-risky-', async () => {
  ...
  it('rolls back when declared contentHash does not match actual content', async () => {
    await withTempProject('kelyra-byoa-rollback-', async () => {
  ...
  it('enforces project policy write limits for external-agent PATCH actions', async () => {
    await withTempProject('kelyra-byoa-policy-limit-', async () => {
      mkdirSync('.kelyra', { recursive: true });
      writeFileSync('.kelyra/policy.json', JSON.stringify({
        version: 1,
        blockedPaths: [],
        confirmPaths: [],
      ...
      const result = await applyExternalAgentActions({
        rawInput: JSON.stringify({
          actions: [{
            ...
            operation: 'PATCH',
            intent: 'MUTATE',
            patches: [{ oldText: 'before', newText: 'after'.repeat(20) }],
            description: 'Oversized policy-limited patch',
          }],
        }),
        saveReceipt: false,
      });
      ...
      assert.equal(result.ok, false);
      assert.match(result.result.errors[0] ?? '', /Large patch writes are blocked/);
      assert.equal(readFileSync('sample.txt', 'utf-8'), 'before');
    });
  });
});

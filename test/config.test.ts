...
describe('Config Constants', () => {
  ...
  it('maps provider display models without showing Claude for forced OpenAI or DeepSeek runs', () => {
    assert.equal(PROVIDER_DEFAULT_MODELS.openai, 'gpt-4o');
    assert.equal(PROVIDER_DEFAULT_MODELS.deepseek, 'deepseek-chat');
    assert.equal(getProviderDisplayModel('openai', 'high'), 'gpt-4o');
    assert.equal(getProviderDisplayModel('deepseek', 'high'), 'deepseek-chat');
    assert.equal(getProviderDisplayModel('anthropic', 'medium'), MODELS.medium);
    assert.equal(getProviderDisplayModel(undefined, 'low'), MODELS.low);
  });

  it('system prompt contains core directives', () => {
    assert.ok(KELYRA_SYSTEM_PROMPT.includes('Strict Write Discipline'));
    assert.ok(KELYRA_SYSTEM_PROMPT.includes('FILE_ACTION'));
    assert.ok(KELYRA_SYSTEM_PROMPT.includes('OPERATION'));
    assert.ok(KELYRA_SYSTEM_PROMPT.includes('Correction Turn'));
  });

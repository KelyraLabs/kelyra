...
describe('kelyra swd apply CLI', () => {
  it('applies stdin FILE_ACTION input as JSON without requiring provider keys', () => {
    const repoRoot = process.cwd();
    const tempDir = mkdtempSync(join(tmpdir(), 'kelyra-swd-cli-'));
...
`;
    ...
    try {
      ...
      assert.equal(existsSync(join(tempDir, '.kelyra', 'receipts', `${parsed.receipt.id}.json`)), true);
    } finally {
  ...
  it('returns machine-readable failure for blocked sensitive files', () => {
    const repoRoot = process.cwd();
    const tempDir = mkdtempSync(join(tmpdir(), 'kelyra-swd-cli-block-'));
    const cliPath = join(repoRoot, 'src', 'cli.ts');
    const tsxLoader = pathToFileURL(join(repoRoot, 'node_modules', 'tsx', 'dist', 'loader.mjs')).href;

...
describe('BANNER (reworked)', () => {
  it('contains the large Kelyra ASCII mark', () => {
  ...
  it('contains the tagline', () => {
    ...
    assert.ok(text.includes('Agent changes you can verify'), 'Should have tagline');
    assert.ok(text.includes('signed receipts'), 'Should mention receipt governance');
  });

  it('does NOT contain hardcoded marketing copy', () => {
    const text = plain(BANNER);
    assert.ok(!text.includes('old provider tier copy'), 'Should not have hardcoded tier');
...
describe('Badge variants', () => {
  it('branchBadge includes branch name', () => {
    const badge = branchBadge('kelyra/session');
    assert.ok(plain(badge).includes('BRANCH: kelyra/session'));
  });

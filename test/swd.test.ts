...
describe('SWDEngine (Production v1 API)', () => {
  ...
  it('applies PATCH actions with explicit oldText/newText replacements', async () => {
    mkdirSync(testDir, { recursive: true });
    const fileA = join(testDir, 'patch.txt');
  ...
  it('blocks oversized PATCH actions before touching disk', async () => {
    mkdirSync(testDir, { recursive: true });
    const fileA = join(testDir, 'large-patch.txt');
    writeFileSync(fileA, 'initial', 'utf-8');
    const engine = new SWDEngine();
    ...
    const result = await engine.run([
      {
        ...
        operation: 'PATCH',
        intent: 'MUTATE',
        patches: [{ oldText: 'initial', newText: 'x'.repeat(MAX_PATCH_BYTES + 1) }],
        description: 'oversized patch',
      },
    ]);

    assert.equal(result.success, false);
    assert.match(result.errors[0] ?? '', /Large patch writes are blocked/);
    assert.equal(readFileSync(fileA, 'utf-8'), 'initial');
    rmSync(testDir, { recursive: true, force: true });
  });

  it('rolls back earlier actions when a later PATCH fails during execution', async () => {
    mkdirSync(testDir, { recursive: true });
    const createdFile = join(testDir, 'created-before-failure.txt');
    const ambiguousFile = join(testDir, 'ambiguous.txt');
    writeFileSync(ambiguousFile, 'repeat\nrepeat\n', 'utf-8');
    const engine = new SWDEngine({ enableRollback: true });
    ...
    const result = await engine.run([
      {
        ...
        description: 'create before later failure',
      },
      {
        path: ambiguousFile,
        operation: 'PATCH',
        intent: 'MUTATE',
        patches: [{ oldText: 'repeat', newText: 'changed' }],
        description: 'ambiguous patch without occurrence',
      },
    ]);
    ...
    assert.equal(readFileSync(ambiguousFile, 'utf-8'), 'repeat\nrepeat\n');
    rmSync(testDir, { recursive: true, force: true });
  });
});

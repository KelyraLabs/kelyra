...
const originalSkillsDir = process.env.KELYRA_SKILLS_DIR;
let tempDir = '';
let globalDir = '';

describe('Skill packs', () => {
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kelyra-skills-project-'));
    globalDir = mkdtempSync(join(tmpdir(), 'kelyra-skills-global-'));
    process.env.KELYRA_SKILLS_DIR = globalDir;
  ...
  afterEach(() => {
    ...
    if (originalSkillsDir === undefined) {
      delete process.env.KELYRA_SKILLS_DIR;
    } else {
      process.env.KELYRA_SKILLS_DIR = originalSkillsDir;
    }

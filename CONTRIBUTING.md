...
```bash
git clone https://github.com/kelyra-labs/kelyra.git
cd kelyra
npm install
cp .env.example .env
...
- Check existing issues before starting larger changes to avoid duplicate work.
...
kelyra is an AI power tool, so AI-assisted contributions are welcome — but they must meet the same quality bar as human code.

...
${c.dim}    Agent changes you can verify: SWD, signed receipts, policy, proof bundles.${c.reset}
`;
...
function testCommandEnv(options?: TestCommandOptions): NodeJS.ProcessEnv {
  if (!options?.sandbox) return process.env;
  ...
  env.KELYRA_TEST_SANDBOX = '1';
  return env;
}

export function runTestCommand(cmd: string, timeoutMsOrOptions: number | TestCommandOptions = 15000): Promise<TestResult> {

...
export const PROVIDER_DEFAULT_MODELS = {
  anthropic: MODELS.high,
  openai: 'gpt-4o',
  deepseek: 'deepseek-chat',
} as const;

export function getProviderDisplayModel(provider: string | undefined, effort: EffortLevel = 'high'): string {
  if (provider === 'openai') return PROVIDER_DEFAULT_MODELS.openai;
  if (provider === 'deepseek') return PROVIDER_DEFAULT_MODELS.deepseek;
  return MODELS[effort] || MODELS.high;
}
...
export const KELYRAIGNORE_FILE = '.kelyraignore';
...
// ── Kelyra System Prompt ─────────────────────────────────────
export const KELYRA_SYSTEM_PROMPT = `\
## IDENTITY
...
export function validateProviderKeys(): AvailableProviders {
  ...
  if (!providers.anthropic && !providers.openai && !providers.deepseek) {
    throw new Error(
      'No model provider API key set. Configure at least one provider for kelyra chat/run:\n' +
      ...
      'Note: kelyra swd apply does not require any model API key.'
    );
  }
  return providers;
}

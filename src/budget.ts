  ...
  formatWarning(): string | null {
    ...
    if (!ok) {
      ...
      return (
        ...
        `${c.green}  Progress saved to MEMORY.md. Resume with ${c.cyan}kelyra chat${c.green} to continue.${c.reset}\n` +
        `${c.dim}  Increase limits: ${c.cyan}kelyra chat --max-tokens 1000000 --max-turns 50${c.reset}\n` +
        `${c.dim}  Disable limits:  ${c.cyan}kelyra chat --no-budget${c.reset}`
      );
    }

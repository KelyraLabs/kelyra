...
export const RECEIPTS_DIR = '.kelyra/receipts';
...
export interface ReceiptChainLink {
  previousId?: string;
  previousIntegritySha256?: string;
}
...
function withIntegrity(receipt: Omit<SWDReceipt, 'integrity'>, signature?: ReceiptSignature): SWDReceipt {
  const integrity: SWDReceipt['integrity'] = { sha256: '' };
  if (signature) integrity.signature = signature;
...
export function createReceiptSigningKeyPair(): ReceiptSigningKeyPair {
...
export function signReceipt(receipt: SWDReceipt, privateKeyPem: string): SWDReceipt {
  const unsigned = withIntegrity(receiptPayload(receipt));
  const privateKey = createPrivateKey(privateKeyPem);
  const publicKeyPem = createPublicKey(privateKey).export({ type: 'spki', format: 'pem' }).toString();
  const signatureBase64 = cryptoSign(
    null,
    Buffer.from(unsigned.integrity!.sha256, 'utf-8'),
    privateKey,
  ).toString('base64');

  return withIntegrity(receiptPayload(unsigned), {
    algorithm: 'ed25519',
    payload: 'integrity.sha256',
    keyId: keyIdForPublicKey(publicKeyPem),
    publicKeyPem,
...
export function verifyReceiptSignature(receipt: SWDReceipt): boolean | null {
  ...
  if (signature.algorithm !== 'ed25519' || signature.payload !== 'integrity.sha256') return false;
  if (!verifyReceiptIntegrity(receipt)) return false;

  try {
    const publicKey = createPublicKey(signature.publicKeyPem);
    return cryptoVerify(
      null,
      Buffer.from(integrity.sha256, 'utf-8'),
      publicKey,
...
function latestReceiptForChain(): SWDReceipt | null {
  const latestPath = receiptFiles()[0];
  return latestPath ? readReceiptFile(latestPath) : null;
}
...
export function createSWDReceipt(input: SWDReceiptInput): SWDReceipt {
  ...
  if (loadPolicy().policy.receiptChain) {
    const previous = latestReceiptForChain();
    if (previous?.integrity?.sha256) {
      base.chain = {
        previousId: previous.id,
        previousIntegritySha256: previous.integrity.sha256,
      };
    }
  }
...
export function verifyReceiptChain(limit = 100): ReceiptChainVerification {
  const receipts = receiptFiles()
    .slice(0, limit)
    .map((filePath) => readReceiptFile(filePath))
    .filter((receipt): receipt is SWDReceipt => receipt !== null);
  const byId = new Map(receipts.map((receipt) => [receipt.id, receipt]));
  const links: ReceiptChainVerification['links'] = [];

  for (const receipt of receipts) {
    const previousId = receipt.chain?.previousId;
    if (!previousId) {
      links.push({
        id: receipt.id,
        ok: true,
        detail: 'No previous receipt link recorded.',
      });
      continue;
    }

    const previous = byId.get(previousId) ?? readReceipt(previousId);
    if (!previous) {
      links.push({
        id: receipt.id,
        previousId,
        ok: false,
        detail: 'Previous receipt was not found.',
      });
      continue;
    }

    const ok = previous.integrity?.sha256 === receipt.chain?.previousIntegritySha256;
    links.push({
      id: receipt.id,
      previousId,
      ok,
      detail: ok ? 'Previous receipt hash matches.' : 'Previous receipt hash mismatch.',
    });
  }

...
export function printCIVerifyReport(report: CIVerifyReport, asJson = false): void {
  ...
  console.log(heading('Kelyra CI Verification'));
  console.log(`  ${c.dim}Mode:${c.reset}     ${report.mode === 'kelyra-receipts' ? 'Kelyra receipt verification' : 'Generic PR review'}`);
  ...
  if (report.receipt.checked) {
    ...
    console.log(`  ${c.dim}Receipts:${c.reset} no changed Kelyra receipts in this diff`);
  }
  ...
  if (!report.receipt.checked) {
    info('Generic mode was used. Add/commit Kelyra receipts only when you intentionally want receipt-level CI verification.');
  }
}

...
import { SWDEngine, parseActions, resolveSafePath, type FileAction, type SWDOptions, type SWDRunResult, type TextPatch } from '../swd.js';
...
function normalizeOperation(value: unknown): FileAction['operation'] | null {
  ...
  if (op === 'CREATE' || op === 'MODIFY' || op === 'DELETE' || op === 'READ' || op === 'PATCH') return op;
  return null;
}
...
function normalizePatch(value: unknown, path: string): TextPatch {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error(`Invalid patch for ${path}: expected an object.`);
  }

  const raw = value as Record<string, unknown>;
  if (typeof raw.oldText !== 'string' || typeof raw.newText !== 'string') {
    throw new Error(`Invalid patch for ${path}: oldText and newText must be strings.`);
  }
  ...
  if (raw.occurrence !== undefined) {
    if (typeof raw.occurrence !== 'number' || !Number.isInteger(raw.occurrence) || raw.occurrence < 1) {
      throw new Error(`Invalid patch for ${path}: occurrence must be a positive integer.`);
    }
    patch.occurrence = raw.occurrence;
  }

  return patch;
}

function normalizePatches(value: unknown, path: string): TextPatch[] {
  if (value === undefined) return [];
  const rawPatches = Array.isArray(value) ? value : [value];
  return rawPatches.map((patch) => normalizePatch(patch, path));
}
...
function normalizeJsonAction(value: unknown): FileAction {
  ...
  const patches = normalizePatches(raw.patches ?? raw.patch, path);
  if (patches.length > 0) {
...
export async function applyExternalAgentActions(options: ApplyExternalAgentOptions): Promise<SWDApplyResult> {
  ...
  const manifest = loadAgentManifest().manifest;
  const agentId = options.agentId ?? input.agent?.id ?? manifest?.id ?? 'bring-your-own-agent';
  ...
  if (policyResult.policy.maxWritableActionContentBytes) {
    engineOptions.maxWritableActionContentBytes = policyResult.policy.maxWritableActionContentBytes;
  }
  if (policyResult.policy.maxPatchBytes) {
    engineOptions.maxPatchBytes = policyResult.policy.maxPatchBytes;
  }
...
function printHumanResult(output: SWDApplyResult): void {
  ...
  if (output.receipt) {
    console.log(`${c.dim}Receipt: ${c.cyan}kelyra receipts show ${output.receipt.id}${c.reset}`);
  }

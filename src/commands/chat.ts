...
import { type EffortLevel, MAX_CORRECTION_RETRIES, KELYRA_SYSTEM_PROMPT, getEffort, getProviderDisplayModel, validateProviderKeys } from '../config.js';
...
import { isTestCommandAllowed, loadPolicy, type KelyraPolicy } from '../policy.js';
import {
  createSWDReceipt,
  saveSWDReceipt,
...
class ChatSession {
  ...
  constructor(options: ChatOptions, ui: ChatUI) {
    ...
    try {
      ...
      const skillResult = buildSkillPrompt(KELYRA_SYSTEM_PROMPT, skills);
    ...
    this.engine = new SWDEngine({
      strict: true,
      enableRollback: true,
      maxWritableActionContentBytes: this.policy.maxWritableActionContentBytes,
      maxPatchBytes: this.policy.maxPatchBytes,
  ...
  public async setupSandbox(): Promise<string | null> {
    ...
    if (current.startsWith('kelyra/')) throw new Error(`Already inside a kelyra branch: ${current}. Nested sandboxing blocked.`);

    const timestampStr = new Date().toISOString().replace(/[-T:]/g, '').slice(0, 12);
    const branchName = `kelyra/${this.options.branch}-${timestampStr}`;
  ...
  public async processInput(input: string): Promise<boolean> {
    ...
    this.ui.startLoading('Kelyra is thinking...');
    ...
    try {
      ...
      // formatTokenUsage takes KelyraResponse, so we map it here
      this.ui.log(`\n${formatTokenUsage({
      ...
      if (needsDream()) {
        this.ui.warn(`\n${c.yellow}💤 Memory approaching capacity. Run ${c.cyan}kelyra dream${c.yellow} to compress.${c.reset}`);
      }
      return handled;
    } catch (err: any) {
      this.ui.stopLoading();
  ...
  private async approveActions(actions: FileAction[], contextLabel: string): Promise<FileAction[]> {
    const review = reviewActions(actions, this.policy);
    ...
    if (approved.length > 0 && touchesCommandSurface(approved, this.policy)) {
  ...
  private async ensureTestCommandIsStillTrusted(cmd: string): Promise<boolean> {
    if (!isTestCommandAllowed(this.policy, cmd)) {
      this.ui.warn(`Test command blocked by project policy: ${cmd}`);
      return false;
    }
  ...
  private saveReceipt(
    ...
    try {
      ...
      this.ui.log(`${c.dim}Receipt: ${c.cyan}kelyra receipts show ${receipt.id}${c.reset}`);
    } catch (err: any) {
  ...
  private async runTestAttempt(cmd: string, attempt: number, maxRetries: number): Promise<{ passed: boolean; output: string }> {
    ...
    const result = await runTestCommand(cmd, {
      sandbox: this.policy.sandboxTestCommands,
    });
  ...
  private async requestTestFix(prompt: string): Promise<string> {
    this.history.push({ role: 'user', content: prompt });
    this.ui.startLoading('Kelyra is fixing tests...');
  ...
  public async finalize(
    ...
    if (repo && !this.options.dryRun) {
      try {
        ...
        // uncommitted work under a generic "kelyra: session end" message.
        if (sandboxBranch && hasUncommittedChanges()) {
          const touchedFiles = Array.from(this.touchedFiles);
          if (touchedFiles.length > 0) {
            commitChanges('kelyra: session end', touchedFiles);
          } else {
            logWarn('Auto-commit skipped: no Kelyra-managed file paths were recorded.');
          }
        }
...
export async function chatCommand(options: ChatOptions): Promise<void> {
  ...
  const cardConfig: SessionCardConfig = {
    ...
    model: getProviderDisplayModel(session.forceProvider, getEffort(options.effort)),
  ...
  const rl = readline.createInterface({
    ...
    prompt: `${c.magenta}${c.bold}kelyra > ${c.reset}`,
  });
...
export async function runCommand(prompt: string, options: RunOptions): Promise<void> {
  ...
  const cardConfig: SessionCardConfig = {
    provider: session.forceProvider ?? 'auto',
    model: getProviderDisplayModel(session.forceProvider, getEffort(runOptions.effort)),

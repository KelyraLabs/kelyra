...
function scaffoldIgnoreFile(force: boolean): ScaffoldResult {
  const target = resolve(process.cwd(), KELYRAIGNORE_FILE);

  if (existsSync(target) && !force) {
    return { file: KELYRAIGNORE_FILE, action: 'exists' };
  }

  const content =
    `# .kelyraignore — Paths excluded from Kelyra scanning\n` +
    `# One pattern per line. Supports exact names and simple *.ext ignores.\n` +
    `# Regenerate defaults: kelyra init --force\n\n` +
    DEFAULT_IGNORE_PATTERNS.join('\n') + '\n';

  writeFileSync(target, content, 'utf-8');
  return { file: KELYRAIGNORE_FILE, action: 'created' };
}
...
function inspectScaffoldState(): ScaffoldResult[] {
  return [
    {
      file: KELYRAIGNORE_FILE,
      action: existsSync(resolve(process.cwd(), KELYRAIGNORE_FILE)) ? 'exists' : 'missing',
    },
    {
      file: 'MEMORY.md',
...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  if (!hasAnyProvider) {
    console.log(
      `\n  ${c.yellow}⚠${c.reset} ${c.bold}No model provider key configured${c.reset}` +
      `${c.dim}. Set at least one key to use ${c.cyan}kelyra chat${c.dim} or ${c.cyan}kelyra run${c.dim}.${c.reset}`,
    );
  }
  ...
  if (checkOnly) {
    if (missing > 0) {
      console.log(`  ${c.yellow}!${c.reset} ${missing} project file(s) missing. Run ${c.cyan}kelyra init${c.reset} to scaffold them.`);
    } else {
  ...
  if (checkOnly && missing > 0) {
    console.log(`  ${c.dim}$${c.reset} ${c.bold}kelyra init${c.reset}           ${c.dim}Create missing project files${c.reset}`);
  }
  console.log(`  ${c.dim}$${c.reset} ${c.bold}kelyra run "..."${c.reset}      ${c.dim}Run one prompt and exit${c.reset}`);
  console.log(`  ${c.dim}$${c.reset} ${c.bold}kelyra chat${c.reset}           ${c.dim}Start an interactive session${c.reset}`);
  console.log(`  ${c.dim}$${c.reset} ${c.bold}kelyra chat --dry-run${c.reset}  ${c.dim}Preview changes without applying${c.reset}`);
  console.log(`  ${c.dim}$${c.reset} ${c.bold}kelyra swd apply --stdin --json${c.reset} ${c.dim}Verify external-agent file actions without a model key${c.reset}`);
  console.log(`  ${c.dim}$${c.reset} ${c.bold}kelyra verify${c.reset}         ${c.dim}Scan codebase for memory drift${c.reset}`);
  console.log(`  ${c.dim}$${c.reset} ${c.bold}kelyra providers check${c.reset} ${c.dim}Check model key readiness${c.reset}`);
  console.log();
  console.log(hr());
}
